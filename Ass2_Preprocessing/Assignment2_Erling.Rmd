...
gridExtra::grid.arrange(grobs = month_plots_list, ncol = 3)
```


## Part D: STL analysis

Sesonal Trend Residual, has inner and outer loop from loess using smoothing
inner loop: updates tred and seasonality, use hyperparams to sepperate trend and seasonality, trend superposition seasonailty increase in curve
outer loop: compute robustness weights, assesing the residuals.
Loess : local regression, idea: for every data point, fit a polynominal to make a linear fit, qudaratic fit
Iteratively improve fit assigning reducd weight to outliers.
Use loess to estimate sesonal and tred components, and residuals
...
If the autocorrelations are small and close to zero, it indicates that the remainders are uncorrelated (resemble white noice)
...
station_list <- c()
for (i in 1:8) {
  station_list <- c(station_list, timeseries_cleaner(get_dc_from_frost(as.character(cut_stations$id[i]))))
}
```
...
```
```{r}
dc_stations[paste("Temp", cut_stations$name[2], sep=".")] <- col1$Temp
head(dc_stations)
```
