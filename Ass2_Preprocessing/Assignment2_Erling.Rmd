...
for (colname in colnames(dc_stations)) {
  dc_stl <- stl(ts(dc_stations[[colname]], frequency = 365), s.window=41, t.window=5, robust=TRUE)
  pl <- autoplot(dc_stl) +
    ggtitle(colname)
  p_list[[colname]] <- pl
}
...
Trend line does not seems to change much over time, but that might be because the data starts from 2017
